=== Easy Webtrends ===
Contributors: jcpeden
Donate link: http://www.wpbackitup.com/plugins/easy-webtrends
Tags: webtrends, analytics, tracking, tagging, monitoring, traffic, monitor
Requires at least: 3.5.0
Tested up to: 4.3.1
Stable tag: 1.1.1

Quickly tag your sites and track your visitors using Webtrends.

== Description ==

= Teack your site with Webtrends =
Easy Webtrends makes using Webtrends with Wordpress easy and straightforward. Once the plugin has been enabled, you can quickly load your tracking scripts through the Wordpress dashboard and start monitoring your traffic.

= Quickly tag your entire site =
Through the Easy Webtrends options panel, you can quickly tag your entire site and the plugin will load these tags on each and every page.

= Load custom tags into your posts and pages =
As well as global tags across your entire site, you can also generate custom tags based on a URL-string match which will load custom tags if the URL of a given page matches a custom rule.

== Screenshots ==
1. After installing the Easy Webtrends and activating on your site, browse to Settings > Easy Webtrends.
2. From the Easy Webtrends settings interface, paste in your Webtrends tracking code and configure the various settings to your liking.
3. Optionally, enable specific tracking tags to load, or disable tracking altogether, when your page URL contains specific strings.

== Installation ==

The plugin is simple to install:

1. Download `easy-webtrends.zip`
1. Unzip
1. Upload `easy-webtrends` directory to your `/wp-content/plugins` directory
1. Go to the plugin management page and enable the plugin

== Upgrade Notice ==

= 1.0.9 =
* Recommended: Cleared redundant javascripts.

= 1.0.9 =
* Recommended: Fixes PHP warning if no rules are loaded.

= 1.0.8 =
* Critical: Fixes paths for components of the plugin.

= 1.0.7 =
* Recommended: Allows Easy Webtrends to run on any domain.

= 1.0.6 =
* Critical: Fixed a number of conflicts between this plugin and another that caused fatal errors on activation

= 1.0.5 =
* Optional: Increased custom tags to limit of 40

= 1.0.4 =
* Critical: Tracking code does not load properly without this update

= 1.0.3 =
* Recommended: WPMS domain mapping will not work without this update

= 1.0.2 =
* Critical: Webtrends will not work without this update

= 1.0.1 =
* Critical: Webtrends will not work without this update

= 1.0 =
* Initial release.

== Installation ==

The plugin is simple to install:

1. Download 'easy-webtrends.zip'
1. Unzip
1. Upload 'easy-webtrends' directory to your '/wp-content/plugins' directory
1. Go to the plugin management page and enable the plugin

== Changelog ==

= 1.1.0 =
* Cleared redundant javascripts.

= 1.0.9 =
* Fixes PHP warning when no rules are loaded.

= 1.0.8 =
* Fixes paths for components of the plugin.

= 1.0.7 =
* Allows Easy Webtrends to run on any domain.

= 1.0.6 =
* Fixed a number of conflicts between this plugin and another that caused fatal errors on activation

= 1.0.5 =
* Increased custom tags to limit of 40

= 1.0.4 =
* Hard coded tracking code and removed support for v9.

= 1.0.3 =
* Modified URL from which webtrends.js is loaded

= 1.0.2 =
* Fixed error with tracking code not being loaded

= 1.0.1 =
* Fixed error with webtrends.min.js not loading

= 1.0 =
* Initial releasering.